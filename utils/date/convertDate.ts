export const convertDate = (raw: string | Date): string => {
	...
	const rawDate = new Date(utcDate.getTime())
	const currentDate = new Date(Date.now() + KST_OFFSET)

	/** 3) 상대 시간 계산 (KST 기준) */
	const diffSec = Math.floor((currentDate.getTime() - rawDate.getTime()) / 1000)
