            ...
            <div className="max-w-2xl mx-auto p-6">
                ...
                <div className="mb-8">
                    ...
                    <div className="text-gray-600 mt-2 text-sm">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2 items-center">
                            <span className="flex items-center gap-1"><User className="h-3 w-3 mr-1 text-[#8FD694]" />{interview?.jobName ?? '-'}</span>
                            <span className="flex items-center gap-1"><Clock className="h-3 w-3 mr-1 text-gray-400" />{interview?.startedAt ? new Date(interview.startedAt).toLocaleString('ko-KR', { timeZone: undefined }) : '-'}</span>
                        </div>
                    </div>
                </div>
                ...
                <Card>
                    ...
                    <CardContent className="flex flex-col items-center justify-center py-8">
                        ...
                        <span className="flex items-center gap-1 mb-4 text-gray-400"><Clock className="h-3 w-3 mr-1 text-gray-400" />{interview?.startedAt ? new Date(interview.startedAt).toLocaleString('ko-KR', { timeZone: undefined }) : '-'}</span>
                        <Progress value={progressValue} className="w-full max-w-md h-2" />
                    </CardContent>
                    <CardFooter className="flex justify-center border-t pt-6">
                        <Button className="bg-[#8FD694] hover:bg-[#7ac47f] text-white" onClick={startInterview}>
                            지금 시작하기
                        </Button>
                    </CardFooter>
                </Card>
