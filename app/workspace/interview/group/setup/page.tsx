...
export default function GroupInterviewSetupPage() {
  ...
  const updateParticipant = (id: number, field: "resume" | "coverLetter", value: string) => {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">다대다 면접을 시작하기 위한 설정을 완료해주세요.</p>
        </div>
        ...
        <div className="mb-8">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="text-gray-500">
              {step === 1 ? "참가자 정보" : step === 2 ? "이력서/자소서 선택" : "면접 옵션 선택"}
            </span>
          </div>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>{step === 1 ? "참가자 정보" : step === 2 ? "이력서/자소서 선택" : "면접 옵션 선택"}</CardTitle>
          </CardHeader>
          <CardContent>
            {step === 1 && (
              <div className="space-y-4">
                <p className="text-sm text-gray-600 mb-4">
                  다대다 면접에 참여할 참가자 명단입니다. 필요한 경우 이름을 수정하세요.
                </p>

                {participants.map((participant, index) => (
                  <div key={participant.id} className="flex items-center space-x-2">
                    <div className="w-8 h-8 rounded-full bg-[#8FD694] bg-opacity-20 flex items-center justify-center">
                      <span className="text-[#8FD694] font-medium">{index + 1}</span>
                    </div>
                    <input
                      type="text"
            ...
            {step === 2 && (
              <div className="space-y-6">
                <p className="text-sm text-gray-600 mb-4">각 참가자의 이력서와 자기소개서를 선택해주세요.</p>

                <Tabs defaultValue={participants[0].id.toString()} className="w-full">
                  <TabsList className="w-full flex mb-4">
                    {participants.map((participant, index) => (
                      <TabsTrigger key={participant.id} value={participant.id.toString()} className="flex-1">
                  ...
                  {participants.map((participant) => (
                    <TabsContent key={participant.id} value={participant.id.toString()} className="space-y-4">
                      <div>
                        <Label className="block text-sm font-medium text-gray-700 mb-2">이력서 선택</Label>
                        <Select
                          value={participant.resume}
                          onValueChange={(value) => updateParticipant(participant.id, "resume", value)}
                        >
                          <SelectTrigger>
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) => updateParticipant(participant.id, "coverLetter", value)}
                        >
                          <SelectTrigger>
            ...
            {step === 3 && (
              <div className="space-y-6">
                <p className="text-sm text-gray-600 mb-4">
                  면접 진행 방식에 대한 옵션을 설정해주세요. 이 설정은 모든 참가자에게 공통으로 적용됩니다.
                </p>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      인성 면접은 가치관과 경험에 관한 질문, 기술 면접은 직무 관련 전문 지식을 평가합니다.
                    </p>
                  </div>
