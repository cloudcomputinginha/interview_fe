...
export default function CreateGroupInterviewPage({ params }: { params: { id: string } }) {
    ...
    <>
      ...
      <div className='min-h-screen bg-gray-50'>
        <div className='max-w-4xl mx-auto p-6'>
          ...
          <Card className='mb-6'>
            <CardHeader>
              <CardTitle>{form.step === 1 ? '면접 일정 설정' : '호스트 자료 및 면접 옵션 설정'}</CardTitle>
            </CardHeader>
            <CardContent>
              {form.step === 1 ? (
                <InterviewStep1
          ...
          {form.step === 2 && (
            <InterviewStepNotice date={form.date} time={form.time} formatDateTime={formatDateTime} />
          )}
        </div>
      </div>
    </>
