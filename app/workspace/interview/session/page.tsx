...
export default function InterviewSessionPage() {
  ...
  }, [isStarted, sessionCtx.session, sessionCtx.currentQuestionIdx, sessionCtx.currentFollowUpIdx])
...
function InterviewSessionContent(
  ...
  const processedQaFlow = qaFlow.map(q => ({ ...q, followUps: q.followUps || [] }))
  const currentQa = processedQaFlow[currentQuestionIdx]
  const isFollowUp = currentFollowUpIdx !== -1
  const currentQuestionText = isFollowUp ? currentQa?.followUps?.[currentFollowUpIdx]?.question : currentQa?.question
  const questionNumber = isFollowUp ? `${currentQuestionIdx + 1}-${currentFollowUpIdx + 1}` : `${currentQuestionIdx + 1}`
  ...
  if (isFinalReportLoading) {
    return <div className="flex items-center justify-center h-full text-lg">최종 리포트를 생성하고 있습니다...</div>
  }
              ...
              >
                {
                  ...
                  isFeedbackLoading ? (
                    <div className="flex items-center justify-center h-24">
                      <div className="text-sm text-gray-400 mr-2">피드백을 생성하고 있습니다...</div>
                      <Loader2 className="animate-spin w-8 h-8 text-[#8FD694]" />
                    </div>
                  ) : isAudioPreloading ? (
                    <div className="flex items-center justify-center h-24">
                      <div className="text-sm text-gray-400 mr-2">오디오를 로드하고 있습니다...</div>
                      <Loader2 className="animate-spin w-8 h-8 text-[#8FD694]" />
                    </div>
                  ) :
                    isQuestionLoading ? (
                      <div className="flex items-center justify-center h-24">
                        <div className="text-sm text-gray-400 mr-2">질문을 생성하고 있습니다...</div>
                        <Loader2 className="animate-spin w-8 h-8 text-[#8FD694]" />
                      </div>
                    ) : (
                      <>
                        <div className="flex items-center text-sm text-gray-400 mb-2">
                          <span>
                            질문 {questionNumber}/{isFollowUp ? currentQa.followUps.length : qaFlow.length}
                          </span>
                        </div>
          ...
          <div className="w-full md:w-80 bg-gray-800 p-4 flex flex-col">
            ...
            <div className="mb-6">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={`text-sm font-medium ${timer < 30 ? 'text-red-400' : ''}`}>{formatTime(timer)}</span>
              </div>
            ...
            <div className="mt-auto">
              {!isAnswering ? (
                ...
                <>
                  <div className="mb-2 text-center text-sm text-[#8FD694]">음성 인식 결과 : {voiceAnswerText}</div>
                  <textarea
