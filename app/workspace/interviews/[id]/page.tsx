...
export default function InterviewRoom() {
	...
	const saveRecordingToIndexedDB = async (
		blob: Blob,
	...
	const nextQuestion = () => {
		if (currentQuestionIndex < MOCK_QUESTIONS.length - 1) {
			...
			// Interview completed
			router.push(`/workspace/interviews/${interviewId}/result`)
		}
	}
		...
		return (
			<div className="min-h-screen flex items-center justify-center">
				<div className="text-center">
					...
					<Button onClick={() => router.push('/workspace/interviews')}>
						면접 목록으로 돌아가기
					</Button>
				</div>
			</div>
					...
					<div>
						...
						<div className="flex items-center space-x-4 mt-1">
							...
							<span className="text-sm text-gray-300">
								{interview.options.interviewStyle === 'technical'
									? '기술 면접'
									: '인성 면접'}
							</span>
						</div>
					...
					<div className="space-y-4">
						<Card className="bg-gray-800 border-gray-700">
							<CardContent className="p-6">
								<h3 className="text-lg font-semibold mb-4 text-[#8FD694]">
									면접 질문
								</h3>
								...
								<div className="space-y-3">
									<div className="text-sm text-gray-300">
										답변 시간: {interview.options.answerDuration}분
									</div>
									...
									{!isRecording &&
										currentQuestionIndex === MOCK_QUESTIONS.length - 1 && (
											<Button
												onClick={() =>
													router.push(
														`/workspace/interviews/${interviewId}/result`
													)
												}
