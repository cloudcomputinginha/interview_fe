	...
	const mutation = useMutation({
		mutationFn: async () => {
			...
			if (!selectedResume || !selectedCoverLetter)
				throw new Error('이력서와 자기소개서를 모두 선택해주세요.')
		...
		onSuccess: () => {
			alert(
				'면접 참여 신청이 완료되었습니다. 면접은 예정된 시간에 자동으로 시작됩니다.'
			)
		...
		<div className="min-h-screen bg-gray-50">
			<div className="max-w-4xl mx-auto p-6">
							...
							<div className="flex items-start">
								...
								<div>
									...
									<p className="text-blue-600 text-sm mt-1">
										면접에 사용할 이력서와 자기소개서를 선택해주세요. 면접은{' '}
										{post?.startedAt &&
											new Date(post?.startedAt).toLocaleString('ko-KR')}
										에 자동으로 시작됩니다.
									</p>
								</div>
							</div>
											...
											{coverLetters.length === 0 ? (
												...
												coverLetters.map(letter => (
													<SelectItem
														...
														value={String(letter.coverletterId)}
													>
				...
				<div className="text-center text-sm text-gray-500">
					<p>
						면접 참여 신청 후에도 면접 시작 전까지 이력서와 자기소개서를 변경할
						수 있습니다. 면접 시작 10분 전부터 대기실에 입장할 수 있습니다.
					</p>
				</div>
			</div>
		</div>
