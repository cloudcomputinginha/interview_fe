...
import { useMultiSessionInterview } from '../../../session/hooks/interview/useMultiSessionInterview'
import { useInterviewMultiWebSocket } from '../../../session/hooks/interview/useInterviewMultiWebSocket'
import {
	MultiRealtimeProvider,
	useInterviewMultiRealtime,
} from '../../../session/hooks/interview/useInterviewMultiRealtime'
import { AnimatePresence, motion } from 'framer-motion'
import { useInterviewMultiSession } from '../../../session/hooks/interview/useInterviewMultiSession'
...
export default function GroupInterviewSessionPage({
	...
	const interviewId = paramsData?.id || '1'
	const sessionId = searchParamsData.sessionId || `session_${interviewId}`
	const participantId = Number(searchParamsData.participantId) || 3

	// 멀티 세션 상태 관리
	const sessionCtx = useInterviewMultiSession({
		sessionId,
		participantId: participantId,
		initialParticipants: [
			{ id: '3', name: '홍길동', isActive: false, isMuted: false },
			{ id: '4', name: '김길동', isActive: false, isMuted: false },
			{ id: '5', name: '이길동', isActive: false, isMuted: false },
		],
	})

	// 멀티 세션 면접 관리
	const interviewCtx = useMultiSessionInterview(
		interviewId,
		participantId,
		sessionCtx
	)
	...
	// WebSocket 연결
	const socketCtx = useInterviewMultiWebSocket({
		sessionId,
		participantId: participantId.toString(),
		onStateUpdate: sessionCtx.handleStateUpdate,
		onSttText: (text, pid, index, fIndex) => {
			console.log('STT 텍스트 수신:', { text, pid, index, fIndex })
		},
		onFinished: () => {
			console.log('면접 종료')
		},
	...
	if (interviewCtx.isLoading) {
		return (
			<div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
				<div className="text-xl font-semibold">
					{interviewCtx.progressMessage}
				</div>
			</div>
		)
	}
		...
		return (
			<div className="min-h-screen flex flex-col gap-3 items-center justify-center bg-gray-900 text-white">
				...
				<Button
					...
					onClick={() => socketCtx.connectWsAsync()}
				>
					연결 시도
				</Button>
			</div>
	...
	if (sessionCtx.state.isFinished) {
		return (
			<div className="min-h-screen flex flex-col gap-3 items-center justify-center bg-gray-900 text-white">
				<div className="text-xl font-semibold mb-4">멀티 세션 면접 종료</div>
			</div>
		)
	}
	...
	return (
		<MultiRealtimeProvider
			wsRef={socketCtx.wsRef}
			connectWsAsync={socketCtx.connectWsAsync}
			wsCloseCode={socketCtx.lastCloseCode}
		>
			<GroupInterviewSessionContent
				interviewCtx={interviewCtx}
						...
						{!isAnswering ? (
							<Button
								className={`w-full text-white ${
									isWsConnected && isCurrentParticipantActive
