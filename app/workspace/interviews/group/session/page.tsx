...
export default function GroupInterviewSessionPage() {
	...
	const nextQuestion = () => {
		if (currentQuestion < questions.length) {
			...
			if (confirm('모든 질문이 끝났습니다. 면접을 종료하시겠습니까?')) {
				window.location.href = '/workspace/interviews/group/report'
			}
		}
	}
	...
	const endInterview = () => {
		if (
			confirm('면접을 종료하시겠습니까? 지금까지의 진행 상황은 저장됩니다.')
		) {
			window.location.href = '/workspace/interviews/group/report'
		}
	}
	...
	const currentParticipant =
		participants.find(p => p.isActive) || participants[0]
						...
						{participants.map(participant => (
							...
							>
								<div className="flex items-center">
									...
									<span
										className={`${participant.isActive ? 'font-medium text-[#8FD694]' : ''}`}
									>
										{participant.name}
									</span>
								</div>
				...
				<div className="flex-1 p-4 flex flex-col">
					...
					<div className="bg-gray-800 p-4 rounded-lg mb-4">
						<div className="flex justify-between items-center mb-2">
							...
							<span
								className={`text-lg font-medium ${timeLeft < 30 ? 'text-red-400' : ''}`}
							>
								{formatTime(timeLeft)}
							</span>
						</div>
