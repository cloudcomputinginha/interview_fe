...
interface InterviewStore {
	...
	addCoverLetter: (
		coverLetter: Omit<CoverLetter, 'id' | 'ownerId' | 'createdAt'>
	) => string
		...
		(set, get) => ({
			...
			startInterview: id => {
				set(state => ({
					interviews: state.interviews.map(interview =>
						interview.id === id
							? { ...interview, status: 'inProgress' as InterviewStatus }
							: interview
					),
				}))
			},
			...
			getScheduledInterviews: () =>
				get().interviews.filter(interview => interview.status === 'scheduled'),
